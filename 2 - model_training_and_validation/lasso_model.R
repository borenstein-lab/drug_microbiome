...
model <- 
  logistic_reg(penalty = .10, mixture = 1) %>%
...
model_cv_results<- fit_resamples(model_rf_workflow, model_data_cv, 
...
model_microb_cv_results<- fit_resamples(model_rf_workflow, model_data_microb_cv, 
                                        control = control_resamples(save_pred = TRUE),
...
model_drug_cv_results<- fit_resamples(model_rf_workflow, model_data_drug_cv, 
                                      control = control_resamples(save_pred = TRUE),
                                      metrics = metric_set(roc_auc, j_index, sens, spec, precision, kap, pr_auc))
